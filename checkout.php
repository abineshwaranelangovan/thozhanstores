<?php
    // Starting session and necessary inclusions
    ob_start();
    session_start();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['agree']) && $_POST['agree'] == 'true') {
            ...
            // Check if user meta exists and insert/update accordingly
...
<script>
    var options = {
        "key": "rzp_test_qgxhzgS6KGGZUD", // Enter the Key ID generated from the Dashboard
        "amount": "<?php echo $total * 100; ?>", // Amount in paise
        "currency": "INR",
        "name": "Thozhan Stores",
        "description": "Order Payment",
        "order_id": "<?php echo isset($razorpayOrderId) ? $razorpayOrderId : ''; ?>", // Pass the `id` obtained from the order creation in the previous step
        ...
        "prefill": {
            "name": "<?php echo $r['firstname'] . ' ' . $r['lastname']; ?>",
            "email": "<?php echo $_SESSION['customer']; ?>",
            "contact": "<?php echo $r['mobile']; ?>"
        },
        "notes": {
            "address": "<?php echo $r['address1'] . ', ' . $r['address2'] . ', ' . $r['city'] . ', ' . $r['state'] . ', ' . $r['zip']; ?>"
        },
        "theme": {
            "color": "#3399cc"
        }
